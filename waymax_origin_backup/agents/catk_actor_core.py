...
class CATK_WaymaxActorCore(abc.ABC):
...
_ActorCore = TypeVar('_ActorCore', bound=CATK_WaymaxActorCore)
...
def actor_core_factory(
    ...
    name: str = 'CATK_WaymaxActorCore',
) -> CATK_WaymaxActorCore:
  """Creates a CATK_WaymaxActorCore from pure functions.
  ...
  class ActorCoreInstance(CATK_WaymaxActorCore):
...
def merge_actions(
  ...
  def _merge_actions(mask, first, second):
    return datatypes.Action(
        data=jnp.where(mask, first.data, second.data),
        valid=jnp.where(mask, first.valid, second.valid),
    )
