...
class LightningTrainer(pl.LightningModule):
    def __init__(
        ...
        self.distance_loss_weight = model.distance_loss_weight
        self.collision_loss_weight = model.collision_loss_weight
        self.cil_loss_weight = model.cil_loss_weight
        self.planning_loss_weight = model.planning_loss_weight
        self.prediction_loss_weight = model.prediction_loss_weight
        self.displcement_loss_weight = model.displcement_loss_weight
        self.no_displacement_for_CLSR = model.no_displacement_for_CLSR
        self.no_prediction_for_CLSR = model.no_prediction_for_CLSR
    ...
    def _compute_objectives(self, res, data) -> Dict[str, torch.Tensor]:
        ...
        # planning loss
        ego_reg_loss, ego_cls_loss, collision_loss, target_r_index, target_m_index = self.get_planning_loss(
        ...
        if self.training and self.use_contrast_loss:
            ...
            contrastive_loss_router = prediction_loss.new_zeros(1)
        else:
            contrastive_loss = prediction_loss.new_zeros(1)
            contrastive_loss_router = prediction_loss.new_zeros(1)

        loss = (
            (ego_reg_loss * self.planning_loss_weight)
            + (ego_cls_loss * self.planning_loss_weight)
            + (prediction_loss * self.prediction_loss_weight)
            + (contrastive_loss*self.cil_loss_weight)
            + (collision_loss*self.collision_loss_weight)
            + contrastive_loss_router
            + ego_ref_free_reg_loss
            + (distance_loss_GT*self.displcement_loss_weight) 
            + (distance_loss_pred*self.displcement_loss_weight)
        )

        return {
            "loss": loss,
            "reg_loss": (ego_reg_loss.item() * self.planning_loss_weight),
            "cls_loss": (ego_cls_loss.item() * self.planning_loss_weight),
            "ref_free_reg_loss": ego_ref_free_reg_loss.item(),
            "collision_loss": (collision_loss.item()*self.collision_loss_weight),
            "prediction_loss": (prediction_loss.item()* self.prediction_loss_weight),
            "contrastive_loss": (contrastive_loss.item()*self.cil_loss_weight),
            "contrastive_loss_router": contrastive_loss_router.item(),
            "distance_loss_GT": (distance_loss_GT.item()*self.displcement_loss_weight),
            "distance_loss_pred": (distance_loss_pred.item()*self.displcement_loss_weight),   
        }
    ...
    def get_planning_loss(self, data, trajectory, probability, valid_mask, target, bs):
        ...
        return reg_loss, cls_loss, collision_loss, target_r_index, target_m_index
