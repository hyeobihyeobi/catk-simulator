    ...
    def forward(self, hidden_states, tgt_route=None):
        ...
        if self.training:
            ...
            for i, expert in enumerate(self.experts): # 여기서도 반복문이네
                y[flat_topk_idx == i] = expert(hidden_states_[flat_topk_idx == i].to(torch.float64)).to(torch.float32) # expert마다 할당된 토큰들에 대해서만 expert 연산 수행
...
class DeepseekMLP(nn.Module):
    ...
    def forward(self, x, tgt_route=None):
        down_proj = self.down_proj(self.act_fn(self.gate_proj(x.to(torch.float32))) * self.up_proj(x.to(torch.float32)))
