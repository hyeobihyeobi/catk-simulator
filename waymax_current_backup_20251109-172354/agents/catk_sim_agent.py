...
from waymax.agents import catk_actor_core

_DEFAULT_CONTROL_FUNC = lambda state: ~state.object_metadata.is_sdc


class CATK_SimAgentActor(catk_actor_core.CATK_WaymaxActorCore):
  ...
  def select_action(
      self,
      params: catk_actor_core.Params,
      state: datatypes.SimulatorState,
      actor_state: Any,
      rng: jax.Array,
  ) -> catk_actor_core.WaymaxActorOutput:
    ...
    return catk_actor_core.WaymaxActorOutput(
        action=action,
        actor_state=None,
...
class FrozenSimPolicy(CATK_SimAgentActor):
