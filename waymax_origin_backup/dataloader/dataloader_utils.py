...
def get_data_generator(
  ...
  if config.distributed:
    assert batch_dims[0] <= jax.local_device_count()
  ...
  if postprocess_fn is None:
    ...
    for example in dataset.as_numpy_iterator():
      # print(type(example['scenario/id']))
      # include scenario id here, not implement in original waymax
      '''debug changing bytes into 32 bit int'''
      import numpy as np
