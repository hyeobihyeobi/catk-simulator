...
class LantentDriver(pl.LightningModule):
    ...
    def forward(
        ...
        flattened_states = states.reshape(batch_size * seq_length, state_elements, state_dims)
        ...
        for i in range(len(self.mpad_blocks)):
            ...
            action_dis, query_content = self.mpad_blocks[i](latent = latent, # B,T,M,hidden (Transformer Decoder Cross Attention)
    ...
    def training_step(self, batch, batch_idx):
        ...
        action_preds,rep_dist, latent_dist = self.forward( # B,T,M,7
            states,
        ...
        loss_all /= len(action_preds) # average over MPAD decoder layers
        loss_all += loss_rec
        logs.update({
