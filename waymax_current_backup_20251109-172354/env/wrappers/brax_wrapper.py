...
# JAX-compat shim: expose jax.linear_util for older Flax versions on new JAX
try:
    import jax  # ensure module is loaded before Flax imports it
    if not hasattr(jax, 'linear_util'):
        from jax._src import linear_util as _lu
        jax.linear_util = _lu  # type: ignore[attr-defined]
except Exception:
    pass
